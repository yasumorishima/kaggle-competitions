...
def agent(obs, config=None):
    ...
    if P["planner"] == "route":
        ...
        if P["assign_rule"] == "global":
            ...
            for i, pos in enumerate(units):
                ...
                for val, tile, op in cand:
                    pairs.append((_rank(pos, val, tile, op, was), i, tile, op))
        ...
        for i, pos in enumerate(units):
            ...
            # care, collect.
            cand.sort(key=lambda c: -_rank(pos, c[0], c[1], c[2], was))
            _, tile, op = cand[0]
            if tile not in shed_here:
                claimed.add(tile)
...
# v18/v19 と同じカレンダー。assign_rule（全体最良の組から埋める割当）を足した main.py で載せ直したもの
