...
P = {
    ...
    # A dropping is the only thing on this farm that is use-it-or-lose-it:
    # the tile carries one boolean and not a count, and
    # `_daily_refresh_animals` sets it True again every night whether or
    # not the animal was fed. Nobody refills the market for it either --
    # FERTILIZER is out of TOWN_CENTER_PRODUCTS and no shop lists it -- so
    # that pool is finite and first-come. The planner still offers the job
    ...
    "rival_supply": 1.0,
    # A one-shot crop is a lump, not a trickle. The stock allowance below is
    # divided by the days that are left, which is right for a tile that pays a
    # little every day and wrong for one whose whole cohort pays on a single
    # day: melon ripens on day 10 and the tile is gone on day 13, so the units
    # it will ever put on the market have to fit inside the allowance once,
    # not inside one day's thirtieth of it. Measured on seed 86000: the ceiling
    # read 6 melon tiles while the curve holds 79 units above three quarters of
    # base, and the other farm sold 72 of them on day 10 at a mean of 242
    # against our 66 on day 12 at 175. 0 keeps today's arithmetic.
    #
    # Measured and refused, seed 86000, both opponents: at 1 the ceiling opens
    # to fourteen tiles, melon units go 66 -> 96 and their mean price goes
    # 175 -> 144, so melon earns +2,284 while strawberry -- which is last in
    # the tile budget and pays it -- loses 6,503. Final money 46,849 -> 41,857
    # against the router and 76,455 -> 61,638 against the twin. The allowance
    # is not being divided wrongly: the rival term is what keeps the plan off
    # a lump the other farm has already sold. Left in at 0 so the arithmetic
    # is not proposed again.
    "lump_span": 0,            # 1 = span a one-shot crop's allowance over its own life
    ...
    # How much of the herd's ration the farm grows rather than buys. The feed
    # term below reserves ceil(head * 1.3 / 0.8) tiles -- twenty-three of the
    # fifty-seven the plan can spend at fourteen head -- and `pick_crop` calls
    # wheat "the worst tile on the farm by revenue and the only one whose
    # absence kills the herd". Both halves of that are true, and they point
    # opposite ways only while the ration has to come off our own tiles.
    # Wheat is the one good whose price does not crash: its glut curve is a
    # log, so four hundred units moved cost five dollars. 1.0 is today's
    # behaviour; below it the freed tiles go to the cash crops and the
    # shortfall is bought at the counter.
    "feed_grow": 1.0,
    ...
    "cap_rule": "demand",
    # Whether a head is judged by the quote while it is bought ("spot", which
    # is what every verdict before 2026-08-27 was measured under) or by the
    # price on the day it first produces ("forward").
    ...
    "sched_veto_from": 0,      # ...only from this day (v53: the veto refuses cows on day 7 at payback 21)
    "sched_herd_floor": (),    # ((from_day, {"COW": n, ...}), ...) raising the calendar head count
    # ((last_day, tiles), ...): through last_day, plan at least this many melon
    # tiles and buy melon seed first. The public router opens with 12 melon
    # tiles on day 0 and sells ~72 melons on day 10 at ~$242 -- first into a
    # pool the town never refills -- which is what pays for its herd. Our
    # market_cap nets the rival out and plans 2, so this was never measured.
    "open_melon": (),
    # Harvest a one-shot crop as soon as it holds max_yield instead of waiting
    # for max_yield_day: a melon watered through ages 6-10 already holds 6 on
    # day 10, and the rival who opens with melon sells into the pool on day 10.
    # True = every one-shot crop; a list of crop names = only those.
    "harvest_at_cap": False,
    # ((last_day, [crops]), ...): through last_day, plan none of these crops.
    # The router's opening is a set, not a melon count: melon and wheat on the
    # field, two cows and two sheep, and no tomato or strawberry until day 5.
    # open_melon alone kept our tomato and strawberry lines, so the 12 melon
    # seeds were paid for with the herd (2026-09-24 sweeps: every variant
    # WORSE). This clears the field lines the router does not open with.
    "open_crop_zero": (),
    # Count heads already on the way per species. The old count subtracts every
    # animal in the shed or in a hand -- and the cows bought a line earlier in
    # the same turn -- from each species' need, so on day 0 the two cows cancel
    # the two sheep of the floor, the second sheep waits for a free turn, and
    # by then the cash has gone on feed (v51, seed 90000: one sheep, $261 left).
    "pending_by_species": -1,  # from this day (-1 = off)
    # Through this day, droppings go to market instead of onto the field. The
    ...
    "rush_items": (),           # items whose carrier heads for the shed at once
    "rush_weight": 1.0,
...
def agent(obs, config=None):
    ...
    def projected_inv(item, days=None):
        """Town stock of `item` `days` ahead -- by default, the day a tile
        planted now would first pay.
        ...
        way their farm actually reaches ours. Anything already grown and still
        standing on either board is counted too: it is produce that has not
        reached the market yet, and a tile census cannot see it.
        """
        ...
        return max(0.0, inv0 + drift * (HORIZON.get(item, 1.0) if days is None
                                        else float(days)))
    ...
    def market_cap(item):
        ...
        """
        span = max(6.0, float(days_left))
        if P["lump_span"] and item in CROPS and not CROPS[item]["ongoing"]:
            # The tile's whole life, which is what `max_lifespan_step` gives it.
            span = max(6.0, float(CROPS[item]["max_yield_day"] + 1))
        if P["cap_rule"] == "forward":
            ...
            room = (max(0.0, demand.get(item, 1.0)
                    ...
                    + headroom(item, projected_inv(item)) / span)
        else:
    ...
    take("WHEAT", max(wheat_floor, math.ceil((herd + pending_animals)
                                              * 1.3 * P["feed_grow"] / RATE["WHEAT"])))
    ...
    for _last, _n in P["open_melon"]:
        if day <= int(_last) and day <= P["plant_last_day"]["MELON"]:
            target["MELON"] = max(target.get("MELON", 0), int(_n))
    for _last, _crops in P["open_crop_zero"]:
        if day <= int(_last):
            for _crop in _crops:
                if _crop in target:
                    target[_crop] = 0
    ...
    fert_keep = 0 if (liquidate or day <= P["fert_sell_days"] or day >= P["fert_sell_from"]) else min(
    ...
    if not liquidate:
        ...
        if any(day <= int(_l) for _l, _n in P["open_melon"]):
            seed_queue.remove("MELON")
            seed_queue.insert(0, "MELON")
        _seeds_bought = 0
        for crop in seed_queue:
            ...
            short = min(short, 5)
            if P["seed_cap_free_tiles"]:
                # A seed only becomes a crop on a tile. Past the tiles that are
                # empty or held by a weed it is cash parked for the season: with
                # the crop dial doubled, 103 strawberry seeds sat unplanted from
                # day 12 while the bank sat under 1,500 (seed 82000, 2026-09-15).
                _free = len(empty_tiles) + len(weeds) - sum(int(v) for v in seeds.values()) - _seeds_bought
                short = min(short, max(0, _free))
    ...
    if day <= P["animal_buy_last_day"] and not liquidate:
        ...
        for item in order:
            a = PRODUCER[item]
            _pend_a = pending
            _by_species = 0 <= P["pending_by_species"] <= day
            if _by_species:
                _pend_a = int(shed.get(a, 0)) + sum(
                    iv.get(a, 0) for iv in invs if isinstance(iv, dict))
            ...
            if sched_forced:
                ...
                need = deficit(item) - _pend_a
            if need <= 0 or room <= 0:
                continue
            cost = ANIMALS[a]["cost"]
            # What this head earns is set by the price on the day it starts
            # producing, not by the quote while it is being bought. Measured on
            # seed 46000 against the top replay: wool quotes 172 on day 13, the
            # calendar buys four sheep, their first fleece is due on day 20 --
            # and wool is 55 by day 18 and 1 by day 19, in a season whose town
            # never opened a yarn store. The spot test passes that purchase
            # comfortably (172 * 1.33 * 10 days = 2,288 against a 600 bar); the
            # forward one is the only one that can refuse it.
            # A head is paid across all of its producing days, not on the
            # first one, so it is priced in the middle of that window: four
            # sheep bought on day 13 are still cutting fleece on day 29, and
            # what decides them is the market over those sixteen days.
            first = ANIMALS[a]["first_yield_day"]
            mid = first + max(0.0, days_left - first) / 2.0
            px = (price_at(item, int(projected_inv(item, mid)))
                  if P["animal_payback_rule"] == "forward" else price(item))
            payback = px * RATE[item] * max(0, days_left - ANIMALS[a]["first_yield_day"])
            # A calendar entry is a plan made in another season. It is allowed
            # to overrule the demand estimate -- that is the search's whole
            # question -- but `sched_veto` decides whether it may also overrule
            # a head that cannot pay for itself in this one.
            if payback < cost * P["animal_payback"] and ((P["sched_veto"] and day >= P["sched_veto_from"]) or not sched_forced):
                continue
    ...
    def pick_crop():
        ...
        need_feed = math.ceil((herd + shed_animals + carried_animals)
                              * 1.3 * P["feed_grow"] / RATE["WHEAT"])
    ...
    def jobs_for(pos, inv):
        ...
        for (x, y, t) in animals:
            ...
            if not t.get("fed_today") and wheat_held > 0:
                ...
                if P["feed_value_rule"] == "escape":
                    ...
                    # the other farm pushed milk to $13: 7 cows escaped on days
                    # 21-23 with 33 wheat in the shed (seed 86000, 2026-09-15).
                    feed_val = (unit_price + price("FERTILIZER")) * urgency
                    if t.get("consecutive_unfed", 0) >= 1:
                        feed_val = max(feed_val, float(a["cost"]))
                out.append((feed_val / (1 + P['dist_weight'] * d), (x, y), "FEED"))
        ...
        for (x, y, t) in plants:
            ...
            if (fert_held > 0 and t.get("fertilized_until_day", -1) < day
                    and P["fert_sell_days"] < day < P["fert_sell_from"]):
        ...
        if P["weed_val_min"] > weed_val:
            weed_val = P["weed_val_min"]
        for (x, y, t) in weeds:
            if (x, y) in claimed or not can_act((x, y)):
                continue
        ...
        for st in sheds:
            ...
            if (unfed > carried_wheat and unfed > wheat_held and stock > 0
                ...
                if P["feed_value_rule"] in ("asset", "escape"):
                    ...
                    # day 22 while 7 hungry cows escaped (seed 82000, 2026-09-15).
                    fetch_val = 0.0
                    for _, _, _t in animals:
                        if _t.get("fed_today"):
                            continue
            ...
            if P["rush_items"]:
                # v60 picks its melons on day 10 by 09:00 but the hands keep
                # them until 15:00, after the rival has sold 60 at the top of
                # the curve. A carried rush item calls its hand home at once.
                rush = sum(price(k) * v for k, v in inv.items()
                           if k in P["rush_items"] and isinstance(v, int))
                if rush > 0:
                    out.append((rush * P["rush_weight"] / (1 + P['dist_weight'] * d), st, "DROP"))
    ...
    def plan_routes():
        ...
        for (x, y, t) in animals:
            if not t.get("fed_today") and wheat_stock > 0:
                ...
                pool.append((_fv * feed_w, (x, y), "FEED"))
                wheat_stock -= 1
        for (x, y, t) in plants:
            cd = CROPS[t["crop"]]
...
# v68: v67 + feed_buy_days 1 (hold one day of bought rations, not two)
SCHEDULE = {"0": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 125, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 1, "DIG_w": 100, "DROP_w": 100, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 100, "PASTURE": 0, "PICKUP_w": 100, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 0, "STRAWBERRY_pct": 100, "TOMATO_pct": 100, "WATER_w": 100, "WHEAT_pct": 100, "hands": 5, "land": 1}, "1": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 125, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 1, "DIG_w": 100, "DROP_w": 100, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 100, "PASTURE": 0, "PICKUP_w": 75, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 0, "STRAWBERRY_pct": 100, "TOMATO_pct": 100, "WATER_w": 100, "WHEAT_pct": 100, "hands": 1, "land": 1}, "10": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 125, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 40, "DROP_w": 125, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 100, "PASTURE": 0, "PICKUP_w": 75, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 4, "STRAWBERRY_pct": 100, "TOMATO_pct": 60, "WATER_w": 100, "WHEAT_pct": 100, "hands": 14, "land": 2}, "11": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 125, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 40, "DROP_w": 125, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 100, "PASTURE": 0, "PICKUP_w": 75, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 4, "STRAWBERRY_pct": 100, "TOMATO_pct": 20, "WATER_w": 100, "WHEAT_pct": 100, "hands": 10, "land": 2}, "12": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 125, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 40, "DROP_w": 125, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 60, "PASTURE": 0, "PICKUP_w": 75, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 4, "STRAWBERRY_pct": 100, "TOMATO_pct": 60, "WATER_w": 100, "WHEAT_pct": 100, "hands": 10, "land": 2}, "13": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 100, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 100, "DROP_w": 100, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 60, "PASTURE": 0, "PICKUP_w": 75, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 4, "STRAWBERRY_pct": 100, "TOMATO_pct": 60, "WATER_w": 100, "WHEAT_pct": 100, "hands": 8, "land": 2}, "14": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 100, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 100, "DROP_w": 100, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 60, "PASTURE": 0, "PICKUP_w": 100, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 4, "STRAWBERRY_pct": 100, "TOMATO_pct": 60, "WATER_w": 100, "WHEAT_pct": 100, "hands": 10, "land": 2}, "15": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 100, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 100, "DROP_w": 100, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 60, "PASTURE": 0, "PICKUP_w": 40, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 4, "STRAWBERRY_pct": 140, "TOMATO_pct": 60, "WATER_w": 100, "WHEAT_pct": 75, "hands": 8, "land": 2}, "16": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 100, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 100, "DROP_w": 100, "FEED_w": 100, "FERTILIZE_w": 60, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 60, "PASTURE": 0, "PICKUP_w": 40, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 4, "STRAWBERRY_pct": 140, "TOMATO_pct": 60, "WATER_w": 100, "WHEAT_pct": 75, "hands": 12, "land": 2}, "17": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 100, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 100, "DROP_w": 100, "FEED_w": 100, "FERTILIZE_w": 60, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 60, "PASTURE": 0, "PICKUP_w": 40, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 4, "STRAWBERRY_pct": 140, "TOMATO_pct": 60, "WATER_w": 100, "WHEAT_pct": 75, "hands": 5, "land": 2}, "18": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 100, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 100, "DROP_w": 100, "FEED_w": 100, "FERTILIZE_w": 60, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 60, "PASTURE": 0, "PICKUP_w": 100, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 4, "STRAWBERRY_pct": 140, "TOMATO_pct": 60, "WATER_w": 100, "WHEAT_pct": 75, "hands": 8, "land": 2}, "19": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 100, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 100, "DROP_w": 100, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 60, "PASTURE": 0, "PICKUP_w": 100, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 5, "STRAWBERRY_pct": 140, "TOMATO_pct": 60, "WATER_w": 100, "WHEAT_pct": 75, "hands": 10, "land": 2}, "2": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 125, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 1, "DIG_w": 40, "DROP_w": 100, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 100, "PASTURE": 0, "PICKUP_w": 75, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 0, "STRAWBERRY_pct": 100, "TOMATO_pct": 100, "WATER_w": 100, "WHEAT_pct": 100, "hands": 2, "land": 1}, "20": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 140, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 100, "DROP_w": 100, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 60, "PASTURE": 0, "PICKUP_w": 160, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 5, "STRAWBERRY_pct": 140, "TOMATO_pct": 60, "WATER_w": 75, "WHEAT_pct": 75, "hands": 11, "land": 2}, "21": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 140, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 100, "DROP_w": 100, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 60, "PASTURE": 0, "PICKUP_w": 160, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 5, "STRAWBERRY_pct": 140, "TOMATO_pct": 60, "WATER_w": 75, "WHEAT_pct": 75, "hands": 10, "land": 2}, "22": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 140, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 100, "DROP_w": 100, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 60, "PASTURE": 0, "PICKUP_w": 120, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 5, "STRAWBERRY_pct": 140, "TOMATO_pct": 100, "WATER_w": 75, "WHEAT_pct": 75, "hands": 7, "land": 2}, "23": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 140, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 100, "DROP_w": 100, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 100, "PASTURE": 0, "PICKUP_w": 80, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 5, "STRAWBERRY_pct": 140, "TOMATO_pct": 100, "WATER_w": 75, "WHEAT_pct": 100, "hands": 10, "land": 2}, "24": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 140, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 100, "DROP_w": 100, "FEED_w": 100, "FERTILIZE_w": 140, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 100, "PASTURE": 0, "PICKUP_w": 80, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 5, "STRAWBERRY_pct": 100, "TOMATO_pct": 100, "WATER_w": 75, "WHEAT_pct": 100, "hands": 10, "land": 2}, "25": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 140, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 100, "DROP_w": 100, "FEED_w": 100, "FERTILIZE_w": 140, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 100, "PASTURE": 0, "PICKUP_w": 80, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 5, "STRAWBERRY_pct": 100, "TOMATO_pct": 100, "WATER_w": 75, "WHEAT_pct": 100, "hands": 11, "land": 2}, "27": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 140, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 100, "DROP_w": 100, "FEED_w": 100, "FERTILIZE_w": 140, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 100, "PASTURE": 0, "PICKUP_w": 20, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 5, "STRAWBERRY_pct": 100, "TOMATO_pct": 100, "WATER_w": 75, "WHEAT_pct": 100, "hands": 10, "land": 2}, "28": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 140, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 100, "DROP_w": 100, "FEED_w": 100, "FERTILIZE_w": 140, "GOOSE": 0, "HARVEST_w": 60, "MELON_pct": 100, "PASTURE": 0, "PICKUP_w": 60, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 5, "STRAWBERRY_pct": 100, "TOMATO_pct": 100, "WATER_w": 75, "WHEAT_pct": 100, "hands": 9, "land": 2}, "29": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 140, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 100, "DROP_w": 100, "FEED_w": 100, "FERTILIZE_w": 140, "GOOSE": 0, "HARVEST_w": 120, "MELON_pct": 100, "PASTURE": 0, "PICKUP_w": 100, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 5, "STRAWBERRY_pct": 100, "TOMATO_pct": 100, "WATER_w": 75, "WHEAT_pct": 100, "hands": 9, "land": 2}, "3": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 125, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 6, "DIG_w": 40, "DROP_w": 125, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 100, "PASTURE": 0, "PICKUP_w": 75, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 0, "STRAWBERRY_pct": 100, "TOMATO_pct": 100, "WATER_w": 100, "WHEAT_pct": 100, "hands": 4, "land": 1}, "4": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 125, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 8, "DIG_w": 40, "DROP_w": 125, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 100, "PASTURE": 0, "PICKUP_w": 75, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 0, "STRAWBERRY_pct": 100, "TOMATO_pct": 100, "WATER_w": 100, "WHEAT_pct": 100, "hands": 4, "land": 1}, "5": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 125, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 8, "DIG_w": 40, "DROP_w": 125, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 100, "PASTURE": 0, "PICKUP_w": 75, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 1, "STRAWBERRY_pct": 100, "TOMATO_pct": 100, "WATER_w": 100, "WHEAT_pct": 100, "hands": 4, "land": 1}, "6": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 125, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 8, "DIG_w": 40, "DROP_w": 125, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 100, "PASTURE": 0, "PICKUP_w": 75, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 3, "STRAWBERRY_pct": 100, "TOMATO_pct": 100, "WATER_w": 100, "WHEAT_pct": 100, "hands": 4, "land": 2}, "7": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 125, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 40, "DROP_w": 125, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 100, "PASTURE": 0, "PICKUP_w": 75, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 4, "STRAWBERRY_pct": 100, "TOMATO_pct": 100, "WATER_w": 100, "WHEAT_pct": 100, "hands": 7, "land": 2}, "8": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 125, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 40, "DROP_w": 125, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 100, "PASTURE": 0, "PICKUP_w": 75, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 4, "STRAWBERRY_pct": 100, "TOMATO_pct": 60, "WATER_w": 100, "WHEAT_pct": 100, "hands": 6, "land": 2}, "9": {"BUILD_COOP_w": 100, "BUILD_PASTURE_w": 125, "CARE_w": 100, "CARROT_pct": 100, "COLLECT_FERTILIZER_w": 100, "COOP": 0, "COW": 9, "DIG_w": 40, "DROP_w": 125, "FEED_w": 100, "FERTILIZE_w": 100, "GOOSE": 0, "HARVEST_w": 100, "MELON_pct": 100, "PASTURE": 0, "PICKUP_w": 75, "PLACE_w": 100, "PLANT_w": 100, "SHEEP": 4, "STRAWBERRY_pct": 100, "TOMATO_pct": 60, "WATER_w": 100, "WHEAT_pct": 100, "hands": 7, "land": 2}}
P.update({'animal_payback_rule': 'forward', 'dist_weight': 0.9, 'feed_buy_days': 1, 'feed_grow': 0.2, 'feed_value_rule': 'asset', 'fert_cap': 0, 'fert_sell_days': 6, 'harvest_at_cap': ['MELON'], 'hc_respect_first': True, 'max_hands': 14, 'open_crop_zero': [[5, ['TOMATO', 'STRAWBERRY', 'CARROT']]], 'open_melon': [[0, 12]], 'opening_days': 9, 'plant_last_day': {'WHEAT': 24, 'CARROT': 25, 'MELON': 8, 'TOMATO': 16, 'STRAWBERRY': 16}, 'reserve_by_item': {'FERTILIZER': 0.0}, 'rush_items': ['MELON', 'WOOL'], 'rush_weight': 3.0, 'sched_hands_scale': 1.1, 'sched_herd_floor': [[0, {'COW': 2, 'SHEEP': 2}], [3, {'COW': 3, 'SHEEP': 2}], [5, {'COW': 4, 'SHEEP': 2}], [6, {'COW': 5, 'SHEEP': 3}], [8, {'COW': 6, 'SHEEP': 3}], [10, {'COW': 6, 'SHEEP': 6}]], 'sched_veto': True, 'sell_whole': ['MELON', 'STRAWBERRY']})
