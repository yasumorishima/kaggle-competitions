...
# 同じカレンダー。task_w_scale（カレンダーの作業ダイヤルに掛ける倍率）込み
...
# --- v25: 人手と仕事の割当を全体最良の組から埋める ---
# 名簿順だと 0 番と若い番号が農場全体の良い升を先に取り、その升の隣に立つ人手が
# 別の升まで歩かされる。"global" は先に全部の組を採点して最良の組から埋める。
# 粘りは名簿順の取り合いへの当て物なので、割当が正しくなると良い割当を妨げる側に回る。
# 第一希望の種が尽きた升では第二候補を撒かず農作業へ回す（global_resow=False）。
#
# 対 agents/v16_best.py・各 96 試合・互いに素な 3 つの seed 帯での実測:
#   seed 3600-3647  +6,098 +/- 2,411
#   seed 3900-3947  +5,551 +/- 2,576
#   seed 4100-4147  +4,554 +/- 2,617
#   逆分散合成       +5,441 +/- 1,461  (t=7.30)
P.update({"assign_rule": "global", "stickiness": 1.0, "global_resow": False})
