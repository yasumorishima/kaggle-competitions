...
- 現在の最新 2 提出＝**v68_sched（2026-09-26 03:08 UTC・run `36213836906`）と v67_sched（2026-09-26 02:29 UTC・run `36211786181`）**。v66 以前は押し出された。
  2026-09-26 は 4 本（5 本目は勝つ候補が無く未使用）。
- LB publicScore（参考）：v58 592.6・v60 622.7・v62 655.6・v63 607.0・v64 646.9＝横ばい（金圏 ≈ 2,900）。
...
### ✅ 10 日目から牛 6・羊 6（2026-09-25・cloud・v64 の上）＝**v65_sched を 09-26 の 1 本目に出す**

- 第 1 回（`36125899964`・96 試合）：h10s +1,065／h10sS +1,077 tie（引き直し +591）、hs105 −2,348・hs115 −4,175 WORSE、mh16 は base と完全一致。router：h10s +2,292・h10sS +3,056 BETTER。
- h10s は router 相手で **5 回連続 BETTER**（v63・v64 上で +3,950／+2,700／+2,292 ほか）・双子は毎回小さく正 ⇒ 192 試合で確認：
...
### v66 の上（2026-09-26・cloud）＝勝つ候補なし

- seed 82000 の traj：v66 は 12 日目に現金 15,872 だが牛 5・羊 3・2 区画のまま（10 日目の床 6/6 は置き場が無く未達）、27 日目に 35,719 を抱える。
  router は 12 日目に 3 区画・牛 9・羊 5・イチゴ 33。L12Hv（12 日目に 3 区画＋牛 9・羊 6＋veto 無し）で形は router と同じになるが、自分の金 33,399（base 40,010）。
- 掃引（`36204727954`・双子の相手 v66）：L12／L12H −2,274 WORSE（2 つ同値＝床は土地が無いと効かない…土地を足しても家畜は増えず）、
  L12Hh／L12h（＋人手 16・1.25 倍）−6,408 WORSE。router も全 WORSE。⇒ **3 区画目はメロン後の現金があっても害**＝完全に閉じた。

### ✅ 餌の小麦は育てるより買う（2026-09-26・cloud・v66 の上）＝**v67_sched を提出**

- 第 1 回（`36206553674`・96 試合）：fg02（`feed_grow 0.2`）+974 tie・引き直し +1,460 HELD、fb1 +653・wf8 −493・fb3 −923 tie、**fg07 −13,421 WORSE**。router：wf8・fg07 WORSE、他 tie（fg02 −223）。
- 192 試合（`36209548150`）：**fg02 勝率 0.66・+1,308 ± 723 BETTER・482000〜 96 試合 +2,069 ± 1,224 HELD**、fg01 +1,144 BETTER、fg03 +647 tie。
- 固定：`agents/v67_sched.py`＝v66＋`feed_grow 0.2`（変種と router 90000／82000 で 1 円一致）。

### ✅ 買い置きの飼料は 1 日分（2026-09-26・cloud・v67 の上）＝**v68_sched を提出**

| 変種（`36211877719`・双子の相手 v67） | 対 v67 双子 96 試合 | 対 router 48 試合 |
|---|---|---|
| **fb1**（`feed_buy_days 1`・v67 は 2） | **勝率 0.70・+2,197 ± 1,437 BETTER・492000〜 +2,005 ± 1,637 HELD** | +2,908 tie |
| wf10（`wheat_floor 10`）／fg0（`feed_grow 0`） | +885／−784 tie | tie |
| wfe3（`wheat_floor_early 3`）／wl20（小麦の終期 20） | −3,258／−2,387 WORSE | wl20 WORSE |

- 固定：`agents/v68_sched.py`＝v67＋`feed_buy_days 1`（変種と router 90000／82000 で 1 円一致）。

### v68 の上（2026-09-26・`36213927131`・双子の相手 v68）＝勝つ候補なし

wf10 +416 tie（引き直し +89）・fg01 −541 tie、fr05（`feed_reserve_days 1`）−1,808・cb250 −1,986・cb60（`cash_buffer`）−4,652 WORSE。router も同傾向。
⇒ 飼料・現金の刻みは v68 の値で局所最適。

## ▶▶ 次の一手

0. ✅ v68_sched 提出済み（09-26 の 4 本目）。次の候補は v68 に双子で勝つもの（09-27 から）。
   土地・群れ・人手・売り方の刻みはほぼ尽きた。残る大差は router のイチゴ単価（52 対 37）と肥料 332 対 76＝家畜日（router 395 対 223）。
   v66 上の掃引（`36143131546`）：swM +865 BETTER だが引き直し +241 NOT CONFIRMED、rM・rw5・s8d12・s8d10 は tie／router 全 tie。v64_sched は提出済み（run `36106020839`）。
...
12 日目の 3 区画目（v66 上でも WORSE）・12 日目からの牛 8・羊 6 の床・veto 無し（q3／hv 系・両相手で WORSE）／0 日目の羊 2 頭目（`pending_by_species`・飼料代が消えて家畜が逃げる）／開幕メロン単品（`open_melon` だけ）／一発作物の早取り（`harvest_at_cap`・メロン限定でも害）／10〜11 日目の 3 区画目＋家畜の積み増し（r1l・r2）。
