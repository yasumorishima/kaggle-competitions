...
# STEP3: multi-scale GR texture (a-trous wavelet) features, as ADDED features.
# Both core axes (GR alignment, structural-surface extrapolation) are saturated in
# the base; only a NEW signal type the model lacks helps (the dipbeam aligner did,
# +0.215). The model currently uses GR only via MATCHING (beam/NCC) + a single
# detrend scale; it has no explicit multi-scale band-pass texture of the toe GR.
# The most-voted public kernel (606 votes) is DWT-based => wavelet GR carries signal
# here. a-trous (stationary) wavelet = differences of Gaussian-smoothed GR at dyadic
# scales -> per-toe-row detail coefficients (local bedding texture), baseline-free
# (a difference removes the per-well GR datum). Intra-well + GR-only => trivially
# fold-safe (no cross-well fit, toe TVT never used).
WAV_SRC = r'''
# === STEP3: multi-scale GR texture (a-trous wavelet detail), additive, fold-safe ===
import numpy as _wnp, pandas as _wpd
from scipy.ndimage import gaussian_filter1d as _wgf
from pathlib import Path as _WPath

_WSCALES=[2,4,8,16,32]

def _wav_one(hw_path):
    wid=_WPath(hw_path).stem.replace('__horizontal_well','')
    try: hw=_wpd.read_csv(hw_path)
    except Exception: return None
    if 'TVT_input' not in hw.columns or 'GR' not in hw.columns: return None
    kn=hw[hw['TVT_input'].notna()]; ev=hw[hw['TVT_input'].isna()]
    if len(ev)==0 or len(kn)<10: return None
    gr=hw['GR'].astype(float).interpolate(limit_direction='both')
    gr=gr.fillna(float(_wnp.nanmean(gr)) if _wnp.isfinite(_wnp.nanmean(gr)) else 0.0).to_numpy(_wnp.float64)
    # a-trous: detail_s = smooth(sigma=s) - smooth(sigma=2s); baseline-free band-pass
    _prev=gr; _det={}
    for s in _WSCALES:
        sm=_wgf(gr,sigma=float(s),mode='nearest')
        _det[s]=(_prev-sm); _prev=sm
    pos=ev.index.to_numpy()                 # RangeIndex => positional into gr
    d={'id':[f'{wid}_{i}' for i in ev.index]}
    for s in _WSCALES:
        d[f'gr_w{s}']=_det[s][pos].astype(_wnp.float32)
    return _wpd.DataFrame(d)

def _wav_feats(paths):
    recs=[r for r in (_wav_one(p) for p in paths) if r is not None]
    return _wpd.concat(recs,ignore_index=True) if recs else _wpd.DataFrame({'id':[]})

print("[WAV] computing multi-scale GR texture features ...")
_wv_tr=_wav_feats(hw_paths); _wv_te=_wav_feats(test_paths)
_wn0=len(train_df); _wm0=len(test_df)
train_df=train_df.merge(_wv_tr,on='id',how='left'); test_df=test_df.merge(_wv_te,on='id',how='left')
assert len(train_df)==_wn0 and len(test_df)==_wm0, "[WAV] merge changed row count"
_wcols=[f'gr_w{s}' for s in _WSCALES]
_wmiss=int(train_df[_wcols].isna().any(axis=1).sum())
print(f"[WAV] merged {len(_wcols)} cols | train NaN-rows={_wmiss}")
for _c in _wcols:
    train_df[_c]=train_df[_c].fillna(0.0).astype(_wnp.float32)
    test_df[_c]=test_df[_c].fillna(0.0).astype(_wnp.float32)
feature_cols=[c for c in train_df.columns if c not in SKIP]
X=train_df[feature_cols]; y=train_df["target"]; g=train_df["well"]; Xt=test_df[feature_cols]
print(f"[WAV] +{len(_wcols)} features (multi-scale GR) | #features now {len(feature_cols)}")
import gc as _wgc; _wgc.collect()
'''
...
def main():
    ...
    # 7. INJECT multi-scale GR texture (wavelet) cell after DIPBEAM.
    cells.insert(i_feat + 3, code_cell(WAV_SRC))
    report.append(f"inserted WAV cell at {i_feat + 3} (after dipbeam cell)")
    ...
    # STEP3 multi-scale GR texture (wavelet) features
    assert "[WAV]" in full and "_WSCALES=[2,4,8,16,32]" in full and "gaussian_filter1d" in full
    assert "def _wav_one(hw_path):" in full and "d[f'gr_w{s}']" in full
