...
# Two leak-free dip sources, emitted SIDE-BY-SIDE (additive; GBT picks/combines):
#   (a) heel-slope dip = d(TVT+Z)/d(horizontal) linear fit over the KNOWN zone
#       (single global slope; the proven 9.978 contributor).
#   (b) surface dip    = step-wise d(S_f*) from the LOO global structural surface
#       _FS, where f* = formation whose surface best tracks the well's known-zone
#       TVT+Z profile. Spatially varying => fixes long-toe wells where one linear
#       heel slope is wrong (the BENCH 46% win-rate failure mode).
import numpy as _qnp, pandas as _qpd
from pathlib import Path as _QPath

def _dipbeam_one(hw_path, is_train):
    ...
    # (a) heel dip = d(TVT+Z)/d(horizontal) over the KNOWN zone (known at test time => leak-free)
    ...
    surf_d=(tvt_dip-last_tvt)        # fallback to heel path if surface unavailable
    surf_match=_qnp.float32(1e30)
    try:
        swid=wid if is_train else None
        Sk=_FS.predict(_qnp.column_stack([kx,ky]),swid)
        struct=ktvt+kz
        best_f=None; best_sd=1e30
        for _f in FORMATIONS:
            _sd=float(_qnp.std(struct-Sk[_f]))
            if _sd<best_sd: best_sd=_sd; best_f=_f
        Sc=_FS.predict(_qnp.column_stack([Xc,Yc]),swid)[best_f]  # len = len(ev)+1
        dS=_qnp.diff(Sc)                                          # predicted d(TVT+Z) per step
        # clip expected index move to a generous band (catches surface extrapolation
        # artifacts outside the train hull; real per-step moves are O(1))
        dexp_surf=_qnp.clip((-dZt+dS)/dtvt,-20.0,20.0).astype(_qnp.float64)
        path_s=_beam_dip_jit(sgr,tw_gr,si,10,20.0,144.0,dexp_surf,2)
        surf_d=(tw_tvt[path_s]-last_tvt)
        surf_match=_qnp.float32(best_sd)
    except Exception:
        pass
    ids=[f'{wid}_{i}' for i in ev.index]
...
print("[DIPBEAM] computing dip-aware beam member (heel + surface dip) ...")
_db_tr=_dipbeam_feats(hw_paths, True); _db_te=_dipbeam_feats(test_paths, False)
...
print(f"[DIPBEAM] +4 features (heel+surf) | #features now {len(feature_cols)}")
_gc.collect()
'''
...
def main():
    ...
    assert "+4 features (heel+surf)" in full
