...
# STEP1-core: make the public beam *track* (beam_ref source) dip-aware. Injected
# into the beam-def cell (def _beam_dip_jit + beam_search_dip wrapper) and the
# build_well BEAMS loop (cons/sm5 tracks recomputed with d_exp = expected dTVT
# index move from heel-slope dip + known dZ/dhoriz). All beam_ref-derived feats
# (tdbc*, hyb_d, beam_*_d, beam_vs_form, sc_vs_beam, signal consensus) inherit
# the geometry. Intra-well => fold-safe; toe TVT never used.
BEAM_DIP_DEFS = r'''
# --- dip-aware beam: transition center = expected dTVT index move (geometry) ---
@njit(cache=True)
def _beam_dip_jit(sgr,tw_gr,si,BS,mc,es,d_exp,W):
    n=len(sgr); nt=len(tw_gr); MAX=BS*(2*W+6)
...
def beam_search_dip(gr_h,tw_tvt,tw_gr,start_tvt,d_exp,bs=10,mc=20.,es=144.,r=2):
    si=_nn(tw_tvt,start_tvt)
    sgr=_smooth(gr_h,float(np.nanmean(tw_gr)),r).astype(np.float64)
    p=_beam_dip_jit(sgr,tw_gr.astype(np.float64),si,bs,float(mc),float(es),d_exp.astype(np.float64),2)
    return tw_tvt[p].astype(np.float32)
_beam_dip_jit(np.random.randn(30),np.random.randn(50),25,8,15.,100.,np.zeros(30),2)
print("dip-aware beam JIT ready")
'''

# build_well BEAMS-loop replacement: compute the per-toe expected index move from
# heel-slope dip (intra-well, leak-free) and recompute cons/sm5 with the dip beam.
BEAM_LOOP_OLD = (
    "    bpaths={}\n"
...
BEAM_LOOP_NEW = (
    "    # dip-aware expected dTVT index move for cons/sm5 (heel-slope dip; intra-well, leak-free)\n"
    "    _adtvt=float(np.median(np.diff(tw_tvt)))\n"
    "    if _adtvt>0 and len(ev)>0 and len(kn)>=2:\n"
    "        _aktvt=kn['TVT_input'].to_numpy(np.float64); _azkn=kn['Z'].to_numpy(np.float64)\n"
    "        _axkn=kn['X'].to_numpy(np.float64); _aykn=kn['Y'].to_numpy(np.float64)\n"
    "        _axev=ev['X'].to_numpy(np.float64); _ayev=ev['Y'].to_numpy(np.float64); _azev=ev['Z'].to_numpy(np.float64)\n"
    "        _ahck=np.concatenate([[0.0],np.cumsum(np.hypot(np.diff(_axkn),np.diff(_aykn)))])\n"
    "        _adip=0.0 if np.std(_ahck)<1e-6 else float(np.polyfit(_ahck,_aktvt+_azkn,1)[0])\n"
    "        _aXc=np.concatenate([[_axkn[-1]],_axev]); _aYc=np.concatenate([[_aykn[-1]],_ayev]); _aZc=np.concatenate([[_azkn[-1]],_azev])\n"
    "        _adexp=((-np.diff(_aZc)+_adip*np.hypot(np.diff(_aXc),np.diff(_aYc)))/_adtvt).astype(np.float64)\n"
    "    else:\n"
    "        _adip=0.0; _adexp=np.zeros(len(ev),np.float64)\n"
    "    bpaths={}\n"
    "    for (bs,mc,es,r,tag) in BEAMS:\n"
    "        if tag in ('cons','sm5'):\n"
    "            bpaths[tag]=beam_search_dip(hgr,tw_tvt,tw_gr,last_tvt,_adexp,bs,mc,es,r)\n"
    "        else:\n"
    "            bpaths[tag]=beam_search(hgr,tw_tvt,tw_gr,last_tvt,bs,mc,es,r)\n"
)
...
def main():
    ...
    # 4b. STEP1-core: inject dip-aware beam defs into the beam-def cell (before warmup)
    i_beam = find_cell(cells, "def beam_search(gr_h,tw_tvt,tw_gr,start_tvt")
    sb = src_str(cells[i_beam])
    warm = 'print("Warming up Numba JIT...")'
    assert warm in sb, "beam warmup anchor not found"
    set_src(cells[i_beam], sb.replace(warm, BEAM_DIP_DEFS.strip("\n") + "\n\n" + warm))
    report.append(f"injected dip-aware beam defs into cell {i_beam}")

    # 4c. STEP1-core: swap cons/sm5 tracks to the dip beam in build_well's BEAMS loop
    i_bw = find_cell(cells, "def build_well(hw_path,tw_path,is_train):")
    sbw = src_str(cells[i_bw])
    assert BEAM_LOOP_OLD in sbw, "BEAMS loop anchor not found verbatim"
    set_src(cells[i_bw], sbw.replace(BEAM_LOOP_OLD, BEAM_LOOP_NEW))
    report.append(f"swapped cons/sm5 -> dip beam in build_well cell {i_bw}")
    ...
    # STEP1-core: dip-aware beam wired into the track that feeds beam_ref
    assert "def beam_search_dip(" in full and "def _beam_dip_jit(sgr,tw_gr,si,BS,mc,es,d_exp,W):" in full
    assert "bpaths[tag]=beam_search_dip(hgr,tw_tvt,tw_gr,last_tvt,_adexp,bs,mc,es,r)" in full
    assert "_adexp=((-np.diff(_aZc)" in full
    # the dip beam must be defined before build_well uses it
    assert full.index("def beam_search_dip(") < full.index("bpaths[tag]=beam_search_dip(")
    print("=== PATCH REPORT ===")
    for r in report:
        print(" -", r)
