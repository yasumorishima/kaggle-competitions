...
- **Next lever (in progress) — a Transformer-encoder sequence member.** With the surface axis mined out, the remaining proven base lever is a 2nd decorrelated *backbone*. A **Transformer encoder** (self-attention — a distinct inductive bias from the dilated-conv TCN, where the recurrent BiLSTM was too TCN-correlated) is injected as a Ridge-stack member, with **windowed attention (W ≤ 512)** so memory stays O(W²) and long toe sequences cannot OOM. The first full run re-confirmed the GBT + TCN base (CV 10.166) then OOM-killed ~27 s into the Transformer cell — diagnosed from the saved partial log as CPython retaining the TCN's freed sequence pool in RSS while the Transformer materialized its own full-matrix copy; fixed by returning the arena to the OS (`malloc_trim`) and computing per-column statistics one column at a time. A `delta_xf` Ridge-OOF diagnostic measures its marginal against the verified base 10.0453. (ROGII private LB is hidden until the 2026-08-05 deadline and the public LB is override-saturated and flat, so GroupKFold CV/OOF — not public score — is the only honest signal for base improvements.)
