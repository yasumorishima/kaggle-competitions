...
- **Refusing to sell under the opening price is worth about 2,700 to relax, and the reason is not the one that was predicted.** Printing the book found two goods falling all season and the prediction was that naming those two would fix it. Naming fertilizer and melon is worth +333. Adding tomato — flat at 60 → 56, so quietly under its own floor for most of the season — takes it to **+2,731**, and dropping the floor entirely is +1,692 with an overlapping interval. Which of those is right is a separate measurement; what is already settled is that a floor written for seven goods was silently stopping sales in three.
