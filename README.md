...
- **Next lever (in progress) — a 2nd decorrelated sequence architecture in the Ridge stack.** Since architectural diversity is what moves the base (the dilated-conv TCN gave the largest single member gain), a **bidirectional LSTM** (recurrent — structurally decorrelated from the TCN) is added as a second sequence member to Pipeline A's `oof_preds`/`test_preds` dicts, GroupKFold(by well) OOF, so the Ridge stack weights it alongside the TCN. A `delta_lstm` Ridge-OOF diagnostic measures its marginal; the gold target is to push the genuine base below the ~9.2 lineage ceiling, since private rank is decided by base quality and the field is clustered there. (ROGII private LB is hidden until the 2026-08-05 deadline, and the public LB is override-saturated and flat — so CV/OOF, not public score, is the only honest signal for base improvements.)
