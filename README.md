...
- **In progress — 3-seed TCN bagging + blend visibility:** the TCN's per-fold variance is large (toe RMSE 9.84–12.14), so the next iteration bags 3 seeds per fold (seed-averaged OOF/test) to damp it, and adds prints of the `Climber` member weights / best score and the Optuna post-processing OOF so the blend gain is measurable. A full run is queued (not yet scored — the verified best remains **9.905**).
- **Open levers (toward the ~5.8 frontier):** typewell cross-attention, formation-top auxiliary multi-task, and richer spatial priors (kriging formation tops, neighbor-well TVT). The train `.png` images are unused by every public notebook **but are absent from the test set, so they cannot feed inference** (dead lever).
