...
| **Re-training fork baseline (this repo)** | **10.224** |
...
- **Open levers (toward the ~5.8 frontier):** the train `.png` images are unused by every public notebook **but are absent from the test set, so they cannot feed inference** (dead lever). The live high-leverage path is a **sequence model** — a TCN (dilated 1D-CNN, U-Net skips) with light cross-attention to the typewell, predicting the toe TVT delta as a seq-to-seq extrapolation over the horizontal log; this captures the long-range trajectory continuity that the flat per-row GBT cannot, and is added as an independent track to the ensemble. Secondary: richer spatial priors (kriging formation tops, neighbor-well TVT).
