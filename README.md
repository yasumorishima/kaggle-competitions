...
- **In progress — surface + TCN stack (`rogii-wellbore-medal/`):** the TCN sequence model is added as a Ridge-stack member on top of the surface base to push CV further (full run scoring).
- **Next levers (CV-driven):** explicit dip features (∂S_f · trajectory azimuth), cross-formation datum consensus, typewell cross-well alignment, and kriging the surface. The train `.png` images are absent from the test set, so they cannot feed inference (dead lever).
