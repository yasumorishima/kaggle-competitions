...
- **Open levers (toward the ~5.8 frontier):** strengthen the TCN's ensemble contribution — multi-seed bagging (fold variance is large, 9.84–12.14), typewell cross-attention, formation-top auxiliary multi-task. Secondary: richer spatial priors (kriging formation tops, neighbor-well TVT). The train `.png` images are unused by every public notebook **but are absent from the test set, so they cannot feed inference** (dead lever).
