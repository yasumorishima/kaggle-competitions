...
**Where it stands (2026-09-24).** The two submissions that will be carried into the final tournament are `agents/v48_sched.py` and `agents/v45_sched.py` (only the latest two count, and the final rating is decided by wins alone, not by money). v48 beats v45 in 0.72 of 96 paired games. Against the strongest public agents it still loses every game: 69,558 against 131,411 over 32 games with a public replay, 85,673 against 169,076 on one seed with the public router. The running log for this competition, including what has been closed and what comes next, is [`kaggriculture/CLAUDE.md`](kaggriculture/CLAUDE.md); day-by-day and per-product diagnostics are in [`kaggriculture/diag/`](kaggriculture/diag).

**Earlier (2026-09-16).** The submitted agent is `agents/v45_sched.py`: the climbed capital calendar, plus a forward-priced veto on livestock that cannot pay for itself, plus three sheep standing on day 0 and no fertilizer held back from the market. Against the agent it replaced it wins 0.91 of 96 paired games; against a structurally different calendar of ours it wins 0.76 of 80. The sheep are the larger half, and they are there because the per-product ledger said so: wool is the best thing a head can produce per day and our first fleece reached the market on day 20 against a strong opponent's day 6. Four sheep is a different question from three — at $500 a head, two thirds of the opening purse in livestock leaves nothing to sow with, and it loses by 28,617.

Two caveats are part of the result rather than footnotes to it. Part of the margin against our own previous agent is denial rather than profit: wool crashes as a square, so fleece from day 6 fills the pool the other farm's day-18 fleece was going to sell into, and on one seed our own money fell while the margin rose. That is why a third, structurally different opponent is in every table — the number worth carrying forward is the +4,823 there, not the +6,778 against ourselves. And a candidate that beats the submitted agent on three disjoint bands and a second opponent on two is still sitting unsubmitted, because its sign tracks how much wheat the opponent sells and the ladder's own wheat exposure has not been measured.
...
**And the rating cannot tell this project's agents apart (2026-09-17).** Pulling every episode for three of our own submissions returns 42 games for one, 229 for another and 38 for the third, with win rates of **0.463, 0.434 and 0.432** — indistinguishable at those counts. A submission's rating is a walk that starts at 600 and stops when a later submission replaces it, and the team's leaderboard score is the maximum over those frozen walks. The **626.2** this project called its personal best for three weeks is a 42-game walk that happened to stop above where it started; the agent that played 229 games and beats it in paired play reads **554.4**, and the newest reads **550.9**. Pairing each episode with the opponent's own reward shows what the ladder is made of: our median **83,519** against theirs **83,110**, a median margin of **−5,234**, and **22,316** needed to turn the median loss. At roughly 40 games a submission and a per-game spread near **35,000**, an improvement worth 3,000–7,000 is invisible there by construction — which is why two candidates measured at +6,835 and +3,129 in sparring left no trace on the board. This withdraws a claim made further down this section: that the offline ruler "reads true on the ladder" because a submission moved 594.7 to 669.5 within four hours. That was forty-odd games of the same walk.
...
- **The strongest public agent's opening cannot be bolted on.** Put side by side day by day, the public router spends its whole purse on day 0 on twelve melon tiles, two cows and two sheep, harvests the melons on day 10 and holds **16,689** that evening against this agent's **319**, then turns it into nine cows, four sheep and a third quadrant. This agent never ran that opening for two reasons: its planting ceiling subtracts the rival's supply and plans two melon tiles, and it only harvests a one-shot crop on its last day (12), although a melon watered from age 6 already holds its maximum of six on day 10. Both were made switchable (off by default, identical to v48 to the coin) and measured: every variant lost, against our own agent over 96 games (margin −2,989 to −6,851) and against the router over 48 (−1,389 to −28,915), the more melon the worse. Harvesting at the cap alone was the worst of all against our own agent (−29,186). A single piece of the opening is not the opening.
- **Climbing a fixed action list on margin overfits just as it did on money.** Two four-to-six-hour links raised the training margin from −22,167 to −16,722 while the held-out margin fell from −23,490 to −25,353, and the result lost 24 of 24 games to the agent it was recorded from. A fixed list cannot react, and the live policy it was copied from punishes that.
...
- **Every interval this project had ever printed was too narrow.** The sweep plays each seed from both sides and then divided the spread by the square root of the *game* count, so "96 games" was 48 season draws counted as 96 independent questions about the knob. The two sides of one draw share the seed the town is drawn from. Averaging the sides into their season first and taking the band over seasons widens every band by up to a factor of sqrt(2) — which changes nothing about the point estimates and quite a lot about which of them were ever allowed to be called better. A second defect in the same function: the `+/-95%` printed beside "mean money" was the spread of `a - b`, a column header sitting over a different quantity's interval.
- **A rating below the field median is either a policy that loses or an agent that errors, and those look identical from outside.** Kaggle's episode service answers `{"submissionId": N}` for one's own submission: 218 real episodes, no errors, reward median 67,462, win rate 0.438. So the agent is not crashing; it loses slightly more than it wins to opponents of its own rating — which also settles what the right sparring partner is. The opponents the ladder actually pairs us with are as strong as the agent itself, not as strong as the published leaders.
- **Read the season product by product before theorising about it.** `_process_market` calls `_commit_unit` as a module global, so wrapping that attribute records every unit that really changed hands. Counting *orders* lies, because a replayed-tape opponent issues many that never fill. Against the strongest public agent the 82,143 gap in final money decomposes as strawberry −24,129, fertilizer −22,785, wool −17,736, wheat −13,456, milk −12,172, melon −5,856, egg −4,671 — the animal complex is 57% of it, and the sharpest single line is timing rather than size: first wool sale on day 20 against their day 6.
- **The shape of the glut curve is what makes a product's strategy.** Melon and wool fall as squares and are gone in 158 and 59 units; wheat and egg fall as logs and never really crash; fertilizer falls linearly and no shop consumes it at all. No shop buys melon either, so melon is a one-shot pool the town never refills — about 16,400 for the first farm to land 72 units on it, and 175 rather than 242 a unit for the farm that arrives two days later. An arithmetic that looked wrong (the one-shot crop's stock allowance is divided by the days left, though its whole cohort pays on a single day) turned out to be right: opening the ceiling from six tiles to fourteen earned melon +2,284 and cost strawberry 6,503. The term that subtracts the rival's supply is what keeps the plan off a lump the other farm has already sold.
- **The farm was not short of labour, and walking was not waste.** A dozen hands cost $376 for 288 actions, so more of them looks free. Scaling the calendar's hand count by 1.25 added 1,075 actions of which exactly 177 became work; the rest walked or passed, and against a different opponent even 1.1 was −7,716. Forcing hands to finish the tile they stand on cut walking from 57% to 51% of actions and cut final money from 89,297 to 62,384 — the value of a work action fell from $36.6 to $24.9, because walking is what buys the *right* job. Both closures say the same thing from opposite sides: the jobs were already sorted by value and all the ones worth doing were being done. The binding constraint is the stock of high-value jobs, not the hands.
...
- **The conversion from money to rating is an upper bound, and was read as an estimate.** "+1,000 of money moves the win rate 5.1 points" was computed by re-scoring 117 real ladder games with *this farm's* money shifted by a constant. A policy change does not shift one farm's money by a constant: the market is shared, so whatever we do to our own harvest also moves what the town pays the opponent. Every target derived from that curve — including "bronze is +23,000 from here" — is therefore optimistic by however much the opponent gains alongside us, which is exactly the quantity the sweep had never recorded.- **Every knob that buys capacity loses, and the package loses more than any of its pieces.** The reading had been that a third field, more hands and planting past the town's cap all failed because the farm was broke when they were measured; the current agent buys its rations at the counter instead of growing them and holds no fertilizer back, so the question looked new. It is not. Against our own previous submission over 96 paired games: a third field **−5,190 ± 1,851**, hands at 1.25 **−5,208 ± 1,730**, the two together with idle-filling **−7,426 ± 2,733**, and with a forced herd on top **−11,792 ± 2,836** — monotone in how much capacity is added. Against the strong published plan over 48: **−8,695**, **−10,169**, **−19,077**. One caveat is not a result: planting past the cap reads −9 ± 15, which is not a small effect but a knob that never fires, because this farm never has idle land to fill.
- **The dropping is the only use-it-or-lose-it thing on the farm, and it is still not worth collecting.** Every surviving animal sets `fertilizer_available` every night whether or not it was fed, the tile carries a boolean rather than a count, and no shop and not the town centre ever buys fertilizer — so the pool is finite and first-come, and the strong opponent takes **331 units for $22,854** while this farm takes 43. Weighting the job above its bare quote makes it worse, not better: at 2.5 the farm's money falls from **73,797 to 36,845** on one seed and its milk from **199 units to 63**, and it sells *fewer* droppings, 15. Narrowing the weight to animals that are fed, cared for and hold nothing to harvest does not save it (**47,403**, milk 99), because the competition is one tile over: a hand beside an idle animal takes a 2.5 × 60 dropping instead of walking three tiles to care for another, whose 202 is discounted to 55 by distance. Care is what doubles the next production, so each dropping costs more than a dropping. The opponent collects 331 because eleven hands can do both; adding hands here is the bullet above.
- **A second visit to a cared-for animal does nothing, which settles by reading what a sweep would have guessed at.** The environment's CARE handler opens with `if tile["cared_today"]: return`. The bonus does accumulate across a species' production interval, because it is only zeroed on a production day, so one visit per animal per day is the whole of it. That answers the knob this project wrote for the question and answers it about the opponent too: the top plan's **967 CARE actions on twelve head** are two thirds no-ops, not a multiplier being missed. Nine cows here returned **199 units of milk** against the 180 that ten fully-cared production events pay, so this farm is already at that ceiling.
- **The opening is not labour-bound or land-bound; it is eleven days spent on the cash floor.** Day by day against the strong plan: **3,000** in hand on day 0, **270** on day 1, **150 / 146 / 139** on days 2–4 — against a `cash_buffer` of **120**. The first cow costs 400 and arrives on **day 5**, the sixth on day 11, and the first milk reaches the market on **day 13**; the opponent stands its cattle on day 0 and sells milk from day 8. Nothing starves and nothing escapes, so the constraint is neither feed nor labour. That floor has never been swept in six weeks of tuning, and the knob that would hold hands back while cash is short has never been set at all.
