...
| Approach | Public LB (RMSE, lower better) |
|---|---|
| Leaderboard top | ~5.785 |
| Public best (DWT/DTW-based clone group, *claimed*) | ~9.25 |
| **GBT + TCN ensemble (this repo, best)** | **9.905** |
| Re-training fork baseline — GBT only | 10.224 |
...
- **Rank reality check:** the 9.905 public LB is rank **~1,440 / 3,127 (top 46%)** — mid-pack. The bronze line is ~7.56 and the leaders ~5.7–7.0, so the whole public-clone lineage (which tops out ~9.25, top ~36%) sits outside the medal zone. Incremental GBT/TCN tuning cannot close a −2.3 RMSE gap; a structural change is required.
...
- **In progress — dip-beam as an intra-well stack member (`rogii-wellbore-surface/` → `rogii-wellbore-medal/`):** the dip-aware beam's toe TVT is emitted as a feature (`tvt_dipbeam_d`) alongside the public beam (the GBT picks per well; the bench wins 46% of wells but by large margins). Computed from each well's *own* known-zone dip, typewell, and trajectory only ⇒ **fold-safe by construction, no cross-well leakage**. Wiring smoke-verified (+2 features merged clean); full GroupKFold CV vs 10.06 next.
- **Next levers (CV-driven):** multi-config dip beams, dip refined by blending heel-slope with the structural-surface gradient, cross-formation datum consensus, kriging the surface. The train `.png` images are absent from the test set, so they cannot feed inference (dead lever).
